...
/**
 * Tests for Strategy, including BuyNowStrategy and Hold10Strategy.
 */
public class StrategyTest {
  ...
  private void reset() {
    ...
    p1 = new Player(hold10);
    p2 = new Player(buyNow);
  ...
  public void testPickDestinations() {
    ...
    assertEquals(picked1, this.hold10.pickDestinations(this.destinations));
    Set<IDestination> picked2 = new HashSet<>(Arrays.asList(
            new Destination(boston, denver), new Destination(chicago, boston)));
    assertEquals(picked2, this.buyNow.pickDestinations(this.destinations));
    try {
      assertEquals(new HashSet<>(), this.hold10.pickDestinations(new HashSet<>()));
      fail();
    } catch (IllegalArgumentException e) {
      assertEquals("Not enough destinations to pick from.", e.getMessage());
    }
    try {
      assertEquals(new HashSet<>(), this.buyNow.pickDestinations(new HashSet<>()));
      fail();
    } catch (IllegalArgumentException e) {
      assertEquals("Not enough destinations to pick from.", e.getMessage());
    }
  ...
  public void testHold10MakeMove() {
    reset();
    assertEquals(new Action(true), this.hold10.makeMove(this.pgs));
    Map<Color, Integer> hand = new HashMap<>();
    hand.put(Color.RED, 5);
    hand.put(Color.GREEN, 3);
    hand.put(Color.BLUE, 5);
    this.pgs = new PlayerGameState(this.ggs, hand, 5, new HashSet<>());
    assertEquals(new Action(CHItoDEN2), this.hold10.makeMove(this.pgs));
  ...
  public void testBuyNowMakeMove() {
    reset();
    assertEquals(new Action(CHItoDEN2), this.buyNow.makeMove(this.pgs));
    this.pgs = new PlayerGameState(this.ggs, new HashMap<>(), 5, new HashSet<>());
    assertEquals(new Action(true), this.buyNow.makeMove(this.pgs));
  }
}
